...
module PackageManager
  class Maven < Base
    ...
    def self.check_status_url(package)
      MavenUrl.from_name(package.name).base
    end
    ...
    def self.mapping(package, depth = 0)
      version_xml = get_pom(package[:group_id], package[:artifact_id], package[:latest_version])
      mapping_from_pom_xml(version_xml, depth).merge({ name: package[:name] })
    end

    def self.mapping_from_pom_xml(version_xml, depth = 0)
      xml = if version_xml.respond_to?("package")
              version_xml.package
            else
              version_xml
            end
    ...
    def self.dependencies(name, version, package)
      pom_file = get_raw(MavenUrl.from_name(name).pom(version))
      Bibliothecary::Parsers::Maven.parse_pom_manifest(pom_file, package[:properties]).map do |dep|
        {
