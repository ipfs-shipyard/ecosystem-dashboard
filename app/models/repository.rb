class Repository < ApplicationRecord
  ...
  scope :with_internal_deps, ->(count = 1) { having("SUM(array_length(direct_internal_dependency_package_ids, 1) + array_length(indirect_internal_dependency_package_ids, 1)) >= ?", count).group(:id) }
